...
function App() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const questionnaireData = await questionariosAPI.getAll();
        setQuestionnaires(questionnaireData);
        
        // Load responses from API if user is logged in
        if (user && user.id) {
          const responseData = await respostasAPI.getByUserId(user.id);
        ...
        try {
          const localQuestionnaires = localStorage.getItem('sr_questionarios');
          if (localQuestionnaires) setQuestionnaires(JSON.parse(localQuestionnaires));
          
          const localResponses = localStorage.getItem('sr_responses');
          if (localResponses) setResponses(JSON.parse(localResponses));
        } catch(e) {
  ...
  async function handleSubmitResponse(resp: any) {
    try {
      // Submit to API
      const savedResponse = await respostasAPI.submit(resp);
